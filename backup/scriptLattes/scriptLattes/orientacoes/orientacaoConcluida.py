...
class OrientacaoConcluida:
	...
	def __init__(self, idMembro, partesDoItem='', idOrientando='', nivelOrientacao=''):
		self.idMembro = set([])
		self.idMembro.add(idMembro)

		if not nivelOrientacao=='':
			nivel = nivelOrientacao
		...
		if not partesDoItem=='':
			...
			partes = self.instituicao.rpartition(" - ")
			if not partes[1]=='':
				self.complemento = partes[0]
				self.instituicao = partes[2]
			else:
				self.complemento = ''
	...
	def html(self, listaDeMembros):
		...
		s+= self.complemento + ' - '     if not self.complemento==''  else ''
